...
# --- migrated from src/prism/benchmarks/calvin/config.py ---
from dataclasses import dataclass
import os
from typing import Mapping, MutableMapping

from prism.eval.calvin_action_protocol import VALID_CALVIN_GRIPPER_MODES
...
class CalvinClientConfig:
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> "CalvinClientConfig":
        ...
        config = cls(
            horizon=env_int(environ, "PRISM_CALVIN_HORIZON", 32),
            ...
            gripper_mode=environ.get("PRISM_CALVIN_GRIPPER_MODE", "passthrough"),
            reset_memory_scope=environ.get("PRISM_CALVIN_RESET_MEMORY_SCOPE", "sequence"),
    ...
    def validate(self) -> None:
        if self.horizon <= 0:
            raise ValueError(f"PRISM_CALVIN_HORIZON must be positive, got {self.horizon}")
        ...
        if self.reset_memory_scope not in VALID_RESET_MEMORY_SCOPES:
            raise ValueError(
                "PRISM_CALVIN_RESET_MEMORY_SCOPE must be one of "
